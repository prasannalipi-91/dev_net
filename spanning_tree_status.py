...
file1 = open('D:/MMC-HC/BMS/07_12_2023_Acivity/activity_log_spanning_tree.txt','w') ## need to change with a proper name
...
for i in file:
    ...
    if x[1] != j:
        ...
        command = "show spanning-tree interface " + x[4]
        print(x[0])
#        print(x[3])
#        print(command)
...
#                k = i.split(" ")
#                print(k[0] +  " " + k[1] + " " + k[2] + " " )
        file1.writelines(x[0] + "," + x[1] + "," + x[4] + "\n")
        file1.writelines(output + "\n")
        print("checking_spanning on " + x[0] + "interface " + x[4])
    else:
        command = "show spanning-tree interface " + x[4]
        output = net_connect.send_command(command)
#        for i in output:
