...
with open('D:/Inventory-tool/hc_switch_status_modified.txt', 'r') as file:
...
for index, element in enumerate(new_lines):
    if pattern.match(element):
        ...
        if section_size > 0:
            ...
            file2 = open('D:/Inventory-tool/hc_switch_status_output.txt', 'a')
...
try:
    ...
    if connection.is_connected():
        ...
        cursor.execute('DROP TABLE IF EXISTS HC_SWITCH_STATUS_DATA')

        cursor.execute('''
            CREATE TABLE IF NOT EXISTS HC_SWITCH_STATUS_DATA (
        ...
        with open('D:/Inventory-tool/hc_switch_status_output.txt', 'r') as file:
            lines = file.readlines()
            for i in lines:
                row_array = i.strip().split(",")
                user_data = tuple(row_array)
                insert_query = 'INSERT INTO HC_SWITCH_STATUS_DATA (DEVICE_IP, DEVICE_NAME, REACHABILITY, VERSION, STACK_STATUS, NUM_LIVE_STACK_SW, NUM_CONF_STACK_SW, MODEL) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)'
...
os.remove('D:/Inventory-tool/hc_switch_status_output.txt')
